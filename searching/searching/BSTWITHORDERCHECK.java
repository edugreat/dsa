...
/*
 * Write a recursive method isOrdered() that takes a Node and two keys min and max as arguments and returns
 * true if all the keys in the tree are between min and max; min and max are indeed the smallest and largest keys in the tree, respectively;
 * and the BST ordering property holds for all keys in the tree; false otherwise.
 * 
 *  
 */
public class BSTWITHORDERCHECK<Key extends Comparable<Key>, Value> {

	//private instance that shows whether a tree is naturally ordered of not
	...
	//private method that returns a node for a given rank argument
		...
		public static void main(String[] args) {
			...
			String in = "ADEQJMTS";
			String[] sp = in.split("");
			BSTWITHORDERCHECK<String, String> bst = new BSTWITHORDERCHECK<>();
			for(String s: sp)
			...
			for(int i = 0; i < bst.size(); i++) {
				boolean y = bst.isOrdered(bst.selectFor(i), min, max);
				System.out.println("is ordered? "+y);
			}
		}
}
